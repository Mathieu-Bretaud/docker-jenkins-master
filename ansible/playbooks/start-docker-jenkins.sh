...
ansible-playbook -s ${FULL_PATH}/site.yml -i ${FULL_PATH}/inventories/jenkins.hosts -t start-docker-jenkins
if [ $? -eq 0 ]
then
    echo "[  OK  ] Starting the Docker Jenkins."
else
    echo "[ ERREUR ] Unable to start the Docker Jenkins !"
    exit 1
fi
